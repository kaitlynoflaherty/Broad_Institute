...
def find_direct_routes(start, stop, data):
    """Function to find a route to get from one stop to another
    Params: start, stop - initial stop, destination stop
    Returns: route - name of route to get from one stop to another"""
    for i in range(len(data['data'])):
        # Iterate through each set of attributes
        new_data = data['data'][i]
        desc = new_data['attributes']['description']
    ...
    # if the routes are the same for each stop
    if route != route2:
        message = 'Sorry, we could not find a direct route!'
...
def main():
    ...
    # Question 3: Get from one stop to another
    start = input('Which stop would you like to start from?')
    destination = input('Which stop is your destination?')
    route = find_direct_routes(start, destination, stops)
    print('You can take the ', route)
